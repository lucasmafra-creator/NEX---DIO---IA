...
def build_facts(message, state=None):
    ...
    if any(x in t for x in ['senha','pin','token','codigo de autenticacao']):
        return {'kind':'security','text':'Credenciais bancárias não estão disponíveis e não devem ser solicitadas.'}
    if 'previsao do tempo' in t or 'clima' in t:
        return {'kind':'out','text':'Pergunta fora do escopo financeiro do protótipo.'}
    if ('ano' in t and any(x in t for x in ['gaste','gasto','gastava'])) or '6 meses' in t or 'seis meses' in t:
        return {'kind':'missing','text':'A base contém somente transações de outubro de 2025; não há dados do período solicitado.'}

    # Intenções explícitas têm prioridade sobre o contexto da pergunta anterior.
    if any(x in t for x in ['saldo','sobrou','sobra','disponivel','entrou','entradas','saiu','saidas','receita','receitas','despesa','despesas']):
    ...
    if 'categoria' in t and any(x in t for x in ['maior','mais gasto','mais gaste']):
        d=totals_by_category(rows); c,v=max(d.items(),key=lambda x:x[1])
        return {'kind':'fact','text':f'Maior categoria de saída: {c}. Total: {brl(v)}.'}

    if cat and ('quanto' in t or 'gaste' in t or 'gasto' in t):
        v=total_category(rows,cat)
        return {'kind':'fact','text':f'Categoria: {cat}. Total calculado: {brl(v)}. Período disponível: outubro de 2025.'}

    # Continuidade somente quando a mensagem realmente indica continuação da categoria.
    if state.get('last_category') and any(x in t for x in ['e nessa categoria','e nela','nessa categoria','dessa categoria','e quanto nessa']):
        c=state['last_category']; v=total_category(rows,c)
    ...
    if any(x in t for x in ['oi','ola','olá','opa','bom dia','boa tarde','boa noite']):
        return {'kind':'greeting','text':'O usuário iniciou uma conversa.'}
...
def answer(message, state=None, use_ai=True):
    ...
    if facts['kind'] in ('security','out','missing','greeting'):
